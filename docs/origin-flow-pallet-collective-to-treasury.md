...
`O` (the origin type) is not a parameter of the struct. It's a parameter of the `impl`. When you write:
...
You never write `EnsureRoot<AccountId, RuntimeOrigin>`. The compiler infers `O = RuntimeOrigin` from the constraint:

```rust
type RejectOrigin: EnsureOrigin<Self::RuntimeOrigin>;
//                               ^^^^^^^^^^^^^^^^^^
//                               the compiler knows O must be this
```

The impl says "I work for any `O: OriginTrait`." The constraint says `O` must be `RuntimeOrigin`. The compiler checks that `RuntimeOrigin` implements `OriginTrait` (it does, generated by `construct_runtime!`), and everything resolves.

This is the same pattern everywhere in FRAME:

- `EitherOfDiverse<L, R>` — the `OuterOrigin` parameter on its `impl` is inferred, not passed.
- `EnsureProportionMoreThan<AccountId, I, N, D>` — the `O` on its `impl` is inferred.
- Any struct with a generic `impl` — the generic resolves from context, not from explicit type parameters.

**Rule of thumb**: if a generic type appears on the `impl` but not on the `struct`, it's inferred by the compiler from the context where the struct is used.
...
By itself, it generates no code. It's a marker — an opt-in signal. When `construct_runtime!` later scans all pallets, it checks which ones have `#[pallet::origin]` and includes them in the global `RuntimeOrigin` enum.

Pallets without `#[pallet::origin]` (like `pallet-balances`) don't get a variant in `RuntimeOrigin`. They use the standard system origins (`Signed`, `Root`, `None`) and nothing more.
...
The macro scans each pallet. It finds that `frame_system` and `pallet_collective` (both instances) have `#[pallet::origin]`. It then auto-generates the global origin enum:
...
pub enum RuntimeOrigin {
    ...
    // pallets without #[pallet::origin] don't appear here
}
```
...
The pallet's Config narrows it:

```rust
type Proposal: Dispatchable<RuntimeOrigin = <Self as Config<I>>::RuntimeOrigin>;
```

This says: "the `Proposal` type's `RuntimeOrigin` must be the same as the pallet's `RuntimeOrigin`." This connects the dispatch system to the origin system.

The runtime concretizes both:

```rust
type RuntimeOrigin = RuntimeOrigin;  // the enum
type Proposal = RuntimeCall;          // the call enum
```
