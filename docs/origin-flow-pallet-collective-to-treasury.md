...
Concretely, given a runtime with these pallets:

```rust
construct_runtime!(
    pub enum Runtime {
        System: frame_system,                           // ← has #[pallet::origin] → RawOrigin(Root, Signed, None)
        Council: pallet_collective::<Instance1>,        // ← has #[pallet::origin] → RawOrigin(Members, Member)
...
The macro generates this `RuntimeOrigin` enum:
...
Pallets without `#[pallet::origin]` (like `pallet-treasury` or `pallet-balances`) don't get a variant in `RuntimeOrigin`. They use the standard system origins (`Signed`, `Root`, `None`) and nothing more.
