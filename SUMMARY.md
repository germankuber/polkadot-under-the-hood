# Summary

* [Introduction](README.md)

## Block Lifecycle

* [Parachain Block Lifecycle](docs/parachain-block-lifecycle.md)
* [Transaction Pool Deep Dive](docs/transaction-pool-deep-dive.md)
* [Collation Submission Pipeline](docs/collation-submission-pipeline.md)

## Node Internals

* [The Overseer Deep Dive](docs/overseer-deep-dive.md)

## Runtime Internals

* [Runtime WASM Injection Deep Dive](docs/runtime-wasm-injection-deep-dive.md)
* [Scale Codec and Runtime Metadata Deep Dive](docs/scale-codec-and-runtime-metadata-deep-dive.md)
* [Storage Deep Dive](docs/storage-deep-dive.md)

## XCM

* [Parachain Inherent Data and XCM Processing](docs/parachain-inherent-data-and-xcm-processing.md)
* [XCM Message Execution Pipeline](docs/xcm-message-execution-pipeline.md)
* [XCM Reserve Transfer Flow](docs/xcm-reserve-transfer-flow.md)

## Governance & Origins

* [Origin Flow: pallet-collective to pallet-treasury](docs/origin-flow-pallet-collective-to-treasury.md)
